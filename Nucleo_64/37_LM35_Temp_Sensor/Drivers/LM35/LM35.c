...
float LM35_Read(uint16_t LM35_Instance, uint8_t *debugValue)
{
    ...
    //Select sensor instance ADC channel
    sConfig.Channel = LM35_CfgParam[LM35_Instance].ADC_CH;
    HAL_ADC_ConfigChannel(&hadc, &sConfig);
    //Start ADC Conversion
    HAL_ADC_Start(&hadc);
    HAL_ADC_PollForConversion(&hadc, 1);
    //get results and convert them to temp. in C_degrees
    AD_RES = HAL_ADC_GetValue(&hadc);
    // LM35 outputs signal with a resolution of 0.0805664 C_degrees
    //10mV per C_degree divided into 12 bit register of ADC
